...
export async function GET(request: NextRequest) {
  try {
    ...
    const qualifiedLeads = completedCalls.filter(c => c.outcome === 'Qualified').length;
    const avgDuration = completedCalls.length > 0
      ? Math.round(completedCalls.reduce((sum, c) => sum + (c.duration || 0), 0) / completedCalls.length)
      : 0;
    const avgSentiment = completedCalls.length > 0
      ? Math.round(completedCalls.reduce((sum, c) => sum + (c.sentiment || 50), 0) / completedCalls.length)
      : 50;
    
    // Calculate AI performance
    const aiSuccessRate = aiCalls.length > 0
      ? Math.round((aiCalls.filter(c => c.status === 'COMPLETED' && !c.transferredToHuman).length / aiCalls.length) * 100)
      : 0;
    const aiQualificationRate = aiCalls.length > 0
      ? Math.round((aiCalls.filter(c => c.outcome === 'Qualified').length / aiCalls.length) * 100)
      : 0;
    ...
    const response = {
      ...
      performance: {
        qualifiedLeads,
        qualificationRate: completedCalls.length > 0 
      ...
      cost: {
        ...
        breakdown: {
          ...
          hybrid: Math.round(hybridCalls.length * costPerCall.hybrid * 100) / 100,
        },
        savings: Math.round((humanCalls.length * costPerCall.human - totalCost) * 100) / 100,
      },
...
function generateTimeSeriesData(calls: any[], groupBy: string) {
  ...
  return Array.from(grouped.entries())
    .map(([date, dateCalls]) => ({
      ...
      avgDuration: dateCalls.length > 0
        ? Math.round(dateCalls.reduce((sum, c) => sum + (c.duration || 0), 0) / dateCalls.length)
        : 0,
      avgSentiment: dateCalls.length > 0
        ? Math.round(dateCalls.reduce((sum, c) => sum + (c.sentiment || 50), 0) / dateCalls.length)
        : 50,
    }))
...
function calculateCampaignMetrics(calls: any[]) {
  ...
  return Array.from(campaignMap.entries())
    .map(([campaignId, campaignCalls]) => ({
      ...
      qualifiedLeads: campaignCalls.filter(c => c.outcome === 'Qualified').length,
      conversionRate: campaignCalls.length > 0
        ? Math.round((campaignCalls.filter(c => c.outcome === 'Qualified').length / campaignCalls.length) * 100)
        : 0,
      avgDuration: campaignCalls.length > 0
        ? Math.round(campaignCalls.reduce((sum, c) => sum + (c.duration || 0), 0) / campaignCalls.length)
        : 0,
    }))
...
function generateObjectionAnalysis(calls: any[]) {
  ...
  const objections = [
    ...
    { type: 'Happy with current solution', count: 132, handled: 78, avgHandleTime: 38 },
    { type: 'Need to think about it', count: 98, handled: 92, avgHandleTime: 25 },
    { type: 'Not the decision maker', count: 87, handled: 95, avgHandleTime: 20 },
