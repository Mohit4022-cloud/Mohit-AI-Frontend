...
async function generatePersonalizedEmail(
  ...
  if (enrichedData.news?.length > 0) {
    const newsItems = enrichedData.news
      .slice(0, 2)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { contactIds, csvData, settings, enableEnrichment, templateId } =
    ...
    if (contactIds && contactIds.length > 0) {
      ...
      contacts = dbContacts.map((contact) => ({
        ...
        company:
          ...
          (contact.customFields as any)?.company ||
          "Unknown",
        title: contact.title || "Unknown",
        industry:
          contact.company?.industry || (contact.customFields as any)?.industry,
        companySize:
          contact.company?.size || (contact.customFields as any)?.companySize,
        website:
          contact.company?.domain || (contact.customFields as any)?.website,
