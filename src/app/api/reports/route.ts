...
function generateMockMetrics(): ReportMetrics {
  ...
  const meetingsCompleted = faker.number.int({
    min: 5,
    max: meetingsScheduled,
  });
  ...
  return {
    ...
    meetingsScheduled,
    meetingsCompleted,
    meetingConversionRate: Math.round(
      (meetingsScheduled / connectedCalls) * 100,
    ),
...
function generateAIInsights(metrics: ReportMetrics): AIInsight[] {
  ...
  if (metrics.emailReplyRate < 10) {
    insights.push({
      ...
      description:
        "Your email reply rate is low. Consider personalizing subject lines and content more.",
      priority: "medium",
  ...
  insights.push({
    ...
    description: `You've made ${metrics.totalCalls} calls and sent ${metrics.emailsSent} emails this period. ${
      metrics.leadVelocity > 5
        ? "Your lead velocity is strong!"
        : "Focus on increasing your activity levels."
    }`,
    priority: "low",
    metric: "leadVelocity",
