...
import { 
  User, Clock, AlertTriangle, Target, BarChart3, 
  Filter, Search, SortAsc, SortDesc, Settings,
  Phone, Mail, MessageSquare, Building, Calendar,
  TrendingUp, Users, Zap, Timer, Play, Pause,
  SkipForward, ChevronRight, RefreshCw, Eye,
  Bot, Brain, Sparkles, Shield, ArrowUp, ArrowDown,
  Grip, GripVertical, Move, MoreVertical, Check,
  X, Info, Bell, Activity, Layers, Route
} from "lucide-react";
...
function SortableItem({ lead, onAction }: { lead: QueuedLead; onAction: (action: string, lead: QueuedLead) => void }) {
  const {
    attributes,
    listeners,
    setNodeRef,
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className={cn("text-sm font-medium", getWaitTimeColor(lead.waitTime))}>
...
export default function QueuePage() {
  ...
  const [newLeadNotifications, setNewLeadNotifications] = useState<QueuedLead[]>([]);
  ...
  useEffect(() => {
    ...
    if (realtimeEnabled) {
      ...
      socketInstance.on('lead_updated', (updatedLead: QueuedLead) => {
        setQueuedLeads(prev => prev.map(lead => 
      ...
      socketInstance.on('metrics_update', (newMetrics: Partial<QueueMetrics>) => {
        setMetrics(prev => ({ ...prev, ...newMetrics }));
      });

      socketInstance.on('escalation_triggered', (data: { lead: QueuedLead, rule: EscalationRule }) => {
        // Logging removed for production
      ...
      },
      {
        ...
        threshold: 90,
        action: 'auto_respond',
        message: 'Thank you for your interest. A senior account executive will contact you within 15 minutes.',
        enabled: false,
        triggerCount: 0
      }
      ];
      ...
      avgWaitTime: Math.round(mockLeads.reduce((sum, lead) => sum + lead.waitTime, 0) / mockLeads.length),
      avgResponseTime: 8.5,
      escalated: 5,
      completed: 127,
      abandoned: 3,
      ...
      queuedLeads.forEach(lead => {
        ...
        switch (rule.condition) {
          ...
          case 'score':
            ...
            break;
          case 'priority':
            shouldEscalate = lead.priority === 'high' && lead.waitTime >= rule.threshold;
            break;
        }
  ...
  const handleLeadAction = (action: string, lead: QueuedLead) => {
    ...
    switch (action) {
      case 'contact':
        setQueuedLeads(prev => prev.map(l => 
      ...
      case 'escalate':
        setQueuedLeads(prev => prev.map(l => 
          l.id === lead.id ? { ...l, priority: 'high' } : l
        ));
        break;
    }
  };
  ...
  const filteredLeads = queuedLeads.filter(lead => {
    ...
    if (filters.search && !`${lead.firstName} ${lead.lastName} ${lead.company} ${lead.email}`.toLowerCase().includes(filters.search.toLowerCase())) {
      return false;
    }
    return true;
  });
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">Manage and prioritize your lead queue with automated escalation</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => socket?.emit('refresh_queue')}>
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="prioritize">Increase Priority</SelectItem>
      ...
      {newLeadNotifications.length > 0 && (
        <div className="fixed top-20 right-4 z-50 space-y-2 max-w-sm">
          {newLeadNotifications.map((lead) => (
            <Alert key={lead.id} className="bg-green-50 border-green-200 animate-slide-in">
              ...
              <AlertDescription>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={lead.priority === 'high' ? 'bg-red-500' : 'bg-blue-500'}>
                    {lead.priority}
                  </Badge>
                </div>
              </AlertDescription>
      ...
      {showAdvancedView ? (
        <AdvancedQueueManager
          queueItems={queuedLeads.map(lead => ({
            ...
            enqueuedAt: new Date(Date.now() - lead.waitTime * 60000).toISOString(),
            waitTime: lead.waitTime,
            channel: lead.source === 'Website' ? 'web' : lead.source === 'Email' ? 'email' : 'phone' as any,
            reason: 'Lead qualification',
            tags: lead.tags || [],
            slaStatus: lead.waitTime > 30 ? 'breached' : lead.waitTime > 20 ? 'at_risk' : 'on_track' as any,
            escalationLevel: 0
          }))}
          onQueueUpdate={(items) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Escalated Today</p>
