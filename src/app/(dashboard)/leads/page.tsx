...
export default function EnhancedLeadsPage() {
  ...
  const [savedAdvancedFilters, setSavedAdvancedFilters] = useState<
    { id: string; name: string; filter: any }[]
  >([]);
  ...
  const assignLeadWithAI = (lead: Lead) => {
    ...
    const repScores = mockReps.map((rep) => {
      ...
      // Industry match
      if (
        lead.enrichedData?.company.industry &&
      ...
      if (companySize) {
        if (companySize === "1000-5000" && rep.team === "Enterprise Sales")
          score += 15;
        else if (companySize === "100-1000" && rep.team === "Mid-Market")
          score += 15;
        else if (parseInt(companySize) < 100 && rep.team === "SMB Sales")
          score += 15;
      }
    ...
    alert(
      `Lead assigned to ${bestMatch.rep.name} (Match Score: ${bestMatch.score.toFixed(0)}%)`,
    );
  };
  ...
  const enrichLead = async (lead: Lead) => {
    ...
    setTimeout(() => {
      ...
      setLeads((prev) =>
        prev.map((l) => (l.id === lead.id ? { ...l, enrichedData } : l)),
      );
  ...
  const handleFileImport = async (file: File) => {
    ...
    try {
      ...
      if (duplicateCheck.length > 0) {
        alert(
          `Found ${duplicateCheck.length} potential duplicates. Skipping duplicates.`,
        );
      }

      const uniqueLeads = importedLeads.filter(
        (importedLead) =>
          !leads.some(
            (existingLead) =>
              existingLead.email === importedLead.email ||
  ...
  const filteredLeads = useMemo(() => {
    ...
    if (advancedFilter) {
      filtered = filtered.filter((lead) =>
        applyAdvancedFilter(lead, advancedFilter),
      );
    } else {
      // Apply basic filters
      ...
      if (filters.score !== "all") {
        ...
        filtered = filtered.filter(
          (lead) =>
            lead.score >= (scoreRange[0] || 0) &&
            lead.score <= (scoreRange[1] || 100),
        );
      }
    }
    ...
    if (debouncedSearch) {
      if (useNaturalLanguage) {
        ...
        filtered = filtered.filter((lead) => {
          ...
          if (
            searchLower.includes("hot") ||
            searchLower.includes("high score")
          ) {
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your inbound leads with AI-powered insights
          </p>
        </div>
          ...
          <Dialog>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new lead with automatic duplicate detection and
                  enrichment
                </DialogDescription>
              </DialogHeader>
              ...
              {showDuplicates && (
                <Alert className="mb-4">
                  ...
                  <AlertDescription>
                    Potential duplicate leads detected. Please review before
                    creating.
                  </AlertDescription>
                </Alert>
              )}
          ...
          {selectedLead && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  360-degree view of {selectedLead.firstName}{" "}
                    ...
                    <CardContent>
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          ...
                          <div className="font-medium">
                            {selectedLead.qualificationData.budget ||
                              "Not specified"}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="font-medium">
                            {selectedLead.qualificationData.timeline ||
                              "Not specified"}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="font-medium">
                            {selectedLead.qualificationData.interest ||
                              "Not specified"}
                          </div>
                        </div>
                      </div>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="font-medium">
                                {selectedLead.enrichedData.company.industry}
                              </div>
                            </div>
                            <div>
                              <Label className="text-xs">Company Size</Label>
                              <div className="font-medium">
                                {selectedLead.enrichedData.company.size}{" "}
                                employees
                              </div>
                            </div>
                            <div>
                              ...
                              <div className="font-medium">
                                {selectedLead.enrichedData.company.revenue}
                              </div>
                            </div>
                            <div>
                              <Label className="text-xs">Location</Label>
                              <div className="font-medium">
                                {selectedLead.enrichedData.company.location}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              {selectedLead.enrichedData.company.technologies.map(
                                (tech) => (
                          ...
                          {selectedLead.enrichedData.company.fundingTotal && (
                            <div className="mt-4">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {selectedLead.enrichedData.company.lastFunding}
                              </div>
                            </div>
                          )}
                        </CardContent>
                        ...
                        <CardContent className="space-y-4">
                          <div>
                            ...
                            <ul className="mt-2 space-y-1">
                              {selectedLead.enrichedData.insights.buyingSignals.map(
                                (signal, idx) => (
                                  <li
                                    key={idx}
                          ...
                          <div>
                            ...
                            <ul className="mt-2 space-y-1">
                              {selectedLead.enrichedData.insights.recentNews.map(
                                (news, idx) => (
                                  <li
                                    key={idx}
                                    className="flex items-start gap-2 text-sm"
                            ...
                            <div key={dup.id} className="border rounded-lg p-3">
                              ...
                              <div className="grid grid-cols-2 gap-2 text-sm">
                                <div>
                                  ...
                                  {dup.lead.firstName} {dup.lead.lastName}
                                </div>
                                <div>
                                  <span className="text-muted-foreground">
                                    Email:
      ...
      <Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a CSV file to bulk import leads. The system will
          ...
          <div className="space-y-4">
            <div className="border-2 border-dashed rounded-lg p-8 text-center">
              ...
              <label htmlFor="csv-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Supported columns: First Name, Last Name, Email, Phone,
                  Company, Job Title
                </p>
              </label>
            </div>
            ...
            <div className="bg-muted/50 rounded-lg p-4">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                ...
                <li>
                  • Duplicate leads will be automatically detected and skipped
                </li>
                <li>
                  • All imported leads will be tagged as &quot;imported&quot;
                </li>
              </ul>
            </div>
          </div>
