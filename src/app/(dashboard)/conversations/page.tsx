...
import { 
  ...
  Mic, MicOff, Volume2, VolumeX, Headphones, Video, VideoOff,
  Mail, MessageSquare, Send, Clock, Calendar, User, Building,
  Star, StarOff, TrendingUp, TrendingDown, AlertTriangle,
  Search, Filter, MoreVertical, Settings, Download, Upload,
  FileText, Copy, Edit, Trash2, Plus, Check, X, Info,
  Bot, Brain, Sparkles, Shield, Zap, Target, BarChart3,
  ThumbsUp, ThumbsDown, Flag, BookOpen, Save, Play, Pause,
  RefreshCw, ChevronLeft, ChevronRight, Maximize2, Minimize2,
  Linkedin, MessageCircle as WhatsApp
} from "lucide-react";
...
export default function ConversationsPage() {
  ...
  useEffect(() => {
    ...
    const mockConversations: Conversation[] = [
      {
        ...
        messages: [
          ...
          },
          {
            id: '2',
            sender: 'agent',
            content: 'Hi Sarah, this is ' + user?.name + ' from Mohit AI. Thanks for taking my call.',
          ...
          },
          {
            id: '3',
            sender: 'lead',
            content: 'Hi! Yes, I was expecting your call. I saw your demo request form.',
      ...
      },
      {
        ...
        messages: [
          ...
          },
          {
            id: '2',
            sender: 'lead',
            content: 'Thanks for the follow-up. Can you send me the pricing information we discussed?',
        ...
        messages: [
          {
            id: '1',
            sender: 'lead',
            content: 'Hi! I saw your message on LinkedIn about your SDR platform. Can you tell me more?',
          ...
          },
          {
            id: '2',
            sender: 'agent',
            content: 'Hello Jessica! Thanks for reaching out. I\'d be happy to tell you more about Mohit AI. Are you currently facing any challenges with your SDR team?',
          ...
          },
          {
            id: '3',
            sender: 'lead',
            content: 'Yes, we\'re struggling with response times and lead qualification. Our team is overwhelmed.',
        ...
        messages: [
          {
            id: '1',
            sender: 'agent',
            content: 'Hi Robert, I noticed you\'re the VP of Sales at Enterprise Solutions. I\'ve been following your company\'s impressive growth and thought you might be interested in how we\'re helping similar companies scale their SDR operations. Would you be open to a brief conversation?',
          ...
          },
          {
            id: '2',
            sender: 'lead',
            content: 'Thanks for reaching out. We are actually evaluating SDR tools right now. What makes Mohit AI different from other solutions?',
    ...
    const mockTemplates: Template[] = [
      ...
      },
      {
        ...
        content: 'I understand budget is a concern. Many of our customers initially had the same worry, but found that the ROI from increased conversions more than justified the investment...',
        variables: [],
        usage: 45,
        effectiveness: 71
      },
  ...
  const toggleHold = () => {
    ...
    socket?.emit('toggle_hold', { callId: callState.callId, onHold: !callState.isOnHold });
  };
  ...
  const applyTemplate = (template: Template) => {
    ...
    if (activeConversation) {
      content = content.replace('{{firstName}}', activeConversation.leadName.split(' ')[0] || '');
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            ...
                            {conv.qualityScore && (
                              <span className={cn(
                                "text-xs font-medium",
                                conv.qualityScore >= 80 ? "text-green-600" : 
                                conv.qualityScore >= 60 ? "text-yellow-600" : "text-red-600"
                              )}>
                        ...
                        <div className="text-2xl">
                          {conv.type === 'call' && <Phone className="h-5 w-5" />}
                          {conv.type === 'email' && <Mail className="h-5 w-5" />}
                          {conv.type === 'chat' && <MessageSquare className="h-5 w-5" />}
                          {conv.type === 'sms' && <MessageSquare className="h-5 w-5" />}
                          {conv.type === 'whatsapp' && <WhatsApp className="h-5 w-5 text-green-600" />}
                          {conv.type === 'linkedin' && <Linkedin className="h-5 w-5 text-blue-600" />}
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="flex items-center gap-2">
                    {callState.isIncoming ? (
                      ...
                      <>
                        ...
                        <Button
                          size="icon"
                          variant={callState.isOnHold ? "secondary" : "outline"}
                          onClick={toggleHold}
                        >
                        ...
                        <div className="flex items-center gap-2 mx-4">
                          ...
                          <Slider
                            value={[callVolume]}
                            onValueChange={([value]) => setCallVolume(value || 50)}
                            max={100}
                            className="w-24"
                          />
                        </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {activeConversation.company} • {activeConversation.leadEmail}
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {activeConversation?.type === 'linkedin' && (
                          <DropdownMenuItem onClick={() => window.open(activeConversation.linkedinProfile, '_blank')}>
                  ...
                  <Card className="mb-4">
                    <CardContent className="p-4">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <h4 className="font-semibold">Response Quality Analysis</h4>
                          <Badge variant={qualityMetrics.overall >= 80 ? "default" : "secondary"}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <Progress value={qualityMetrics.responseTime} className="h-2 mt-1" />
                          </div>
                          <div>
                            <Label className="text-xs">Sentiment</Label>
                            <Progress value={qualityMetrics.sentiment} className="h-2 mt-1" />
                          </div>
                          <div>
                            <Label className="text-xs">Relevance</Label>
                            <Progress value={qualityMetrics.relevance} className="h-2 mt-1" />
                          </div>
                          <div>
                            <Label className="text-xs">Grammar</Label>
                            <Progress value={qualityMetrics.grammar} className="h-2 mt-1" />
                          </div>
                          <div>
                            <Label className="text-xs">Persuasiveness</Label>
                            <Progress value={qualityMetrics.persuasiveness} className="h-2 mt-1" />
                          </div>
                        </div>
                        ...
                        >
                          ...
                          {activeConversation?.type === 'linkedin' && (
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs font-medium">LinkedIn Message</span>
                            </div>
                          )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <Textarea
                      ...
                      placeholder={
                        activeConversation?.type === 'whatsapp' ? "Type a WhatsApp message..." :
                        activeConversation?.type === 'linkedin' ? "Type a LinkedIn message..." :
                        activeConversation?.type === 'email' ? "Type your email..." :
                        "Type your message..."
                      }
                      className="flex-1"
                      rows={3}
