...
export async function getAllContacts(params: ContactQueryParams = {}) {
  ...
  if (minLeadScore !== undefined) {
    filteredContacts = filteredContacts.filter(
      (c) => c.leadScore >= minLeadScore,
    );
  }

  if (maxLeadScore !== undefined) {
    filteredContacts = filteredContacts.filter(
      (c) => c.leadScore <= maxLeadScore,
    );
  }
 ...
 */
export async function getContactsByAssignee(
  userId: string,
): Promise<Contact[]> {
...
export async function getContactStats() {
  ...
  const statusCounts = contactsDatabase.reduce(
    (acc, contact) => {
  ...
  const averageLeadScore =
    contactsDatabase.reduce((sum, c) => sum + c.leadScore, 0) / totalContacts ||
    0;

  const sourceCounts = contactsDatabase.reduce(
    (acc, contact) => {
