...
import { 
  FlaskConical, BarChart3, TrendingUp, TrendingDown, Users,
  Mail, MessageSquare, Phone, Clock, Calendar as CalendarIcon,
  Play, Pause, StopCircle, CheckCircle, AlertCircle, Info,
  Copy, Edit, Trash2, Plus, Zap, Target, Sparkles,
  ArrowUp, ArrowDown, Minus, Filter, Search, Download,
  Eye, EyeOff, RefreshCw, Settings, MoreVertical,
  ChevronRight, ExternalLink, GitBranch, Percent, Activity
} from "lucide-react";
...
export function ABTesting({ className }: ABTestingProps) {
      ...
      variants: [
        {
          ...
          allocation: 50,
          content: {
            script: 'Hi {firstName}, I\'m calling to learn about your current SDR process...'
          },
          metrics: {
            sent: 500,
            opened: 500,
        ...
        },
        {
          ...
          allocation: 50,
          content: {
            script: 'Hi {firstName}, I can show you how to double your SDR productivity...'
          },
          metrics: {
            sent: 500,
            opened: 500,
  ...
  const calculateStatisticalSignificance = (variantA: Variant, variantB: Variant) => {
    ...
    const pooled = (variantA.metrics.converted + variantB.metrics.converted) / 
                   (variantA.metrics.sent + variantB.metrics.sent);
    const se = Math.sqrt(pooled * (1 - pooled) * (1/variantA.metrics.sent + 1/variantB.metrics.sent));
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge 
                            variant="outline" 
                            className={cn("text-xs", getStatusColor(test.status))}
                          >
                            {test.status}
                          </Badge>
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>Started {formatDistanceToNow(new Date(test.startDate))} ago</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <Progress 
                          value={(test.sampleSize.current / test.sampleSize.required) * 100} 
                          className="w-32"
                        />
                        <p className="text-xs text-muted-foreground mt-1">
                          {test.sampleSize.current} / {test.sampleSize.required} samples
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <TabsContent value="active" className="space-y-4">
          {selectedTest && selectedTest.status === 'running' ? (
                        ...
                        <div key={variant.id} className="border rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div>
                              ...
                              <p className="font-semibold">{variant.performance.openRate}%</p>
                            </div>
                            <div>
                              <p className="text-muted-foreground">Click Rate</p>
                              <p className="font-semibold">{variant.performance.clickRate}%</p>
                            </div>
                            <div>
                              <p className="text-muted-foreground">Conversion</p>
                              <p className="font-semibold">{variant.performance.conversionRate}%</p>
                            </div>
                            <div>
                              <p className="text-muted-foreground">Revenue/Lead</p>
                              <p className="font-semibold">${variant.performance.revenuePerLead}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold">{selectedTest.confidence}%</p>
                            </div>
                            <Target className="h-8 w-8 text-blue-600" />
                          </div>
                          <Progress value={selectedTest.confidence} className="mt-2" />
                        </CardContent>
                      </Card>
                      <Card>
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm text-muted-foreground">Sample Progress</p>
                              <p className="text-2xl font-bold">
                                {Math.round((selectedTest.sampleSize.current / selectedTest.sampleSize.required) * 100)}%
                              </p>
                            </div>
                            <Users className="h-8 w-8 text-green-600" />
                          </div>
                          <Progress 
                            value={(selectedTest.sampleSize.current / selectedTest.sampleSize.required) * 100} 
                            className="mt-2"
                          />
                        </CardContent>
                      </Card>
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold">
                                {Math.max(0, 
                                  Math.ceil((selectedTest.sampleSize.required - selectedTest.sampleSize.current) / 
                                  (selectedTest.sampleSize.current / differenceInDays(new Date(), new Date(selectedTest.startDate))))
                                )}
                              </p>
                            </div>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="font-semibold text-lg mb-2">No Active Test Selected</h3>
              <p className="text-muted-foreground">
                  ...
                  <div key={test.id} className="border rounded-lg p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Ran from {format(new Date(test.startDate), 'MMM dd')} to{' '}
                          {test.endDate && format(new Date(test.endDate), 'MMM dd, yyyy')}
                        </p>
                      </div>
                      ...
                      {test.variants.map((variant) => (
                        <div 
                          ...
                          className={cn(
                            ...
                            test.winner === variant.id && "border-green-600 bg-green-50"
                          )}
                        >
                          <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-sm">
                              <span className="text-muted-foreground">Revenue per Lead</span>
                              <span className="font-medium">${variant.performance.revenuePerLead}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                    ...
                    <div className="bg-muted rounded-lg p-4">
                      ...
                      <ul className="space-y-1 text-sm">
                        <li className="flex items-center gap-2">
                          ...
                          <span>
                            Winner achieved {Math.round(
                              ((test.variants.find(v => v.id === test.winner)?.performance.conversionRate || 0) /
                              (test.variants.find(v => v.id !== test.winner)?.performance.conversionRate || 1) - 1) * 100
                            )}% higher conversion rate
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                          <span>Statistical significance: {test.confidence}%</span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                          <span>
                            ...
                            {Math.round(
                              (test.variants.find(v => v.id === test.winner)?.metrics.revenue || 0) * 12 / 
                              (differenceInDays(new Date(test.endDate!), new Date(test.startDate)) / 30)
                            ).toLocaleString()}
                          </span>
                        </li>
                      </ul>
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge className="bg-green-600">+45% conversion</Badge>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm">Pain Points First</span>
                          <Badge className="bg-green-600">+38% conversion</Badge>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm">Social Proof</span>
                          <Badge className="bg-green-600">+31% conversion</Badge>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <Alert>
                    ...
                    <AlertDescription>
                      Your email tests show 3x better results when testing subject lines vs body content. 
                      Consider focusing more tests on subject line optimization.
                    </AlertDescription>
                  </Alert>
                  <Alert>
                    <TrendingUp className="h-4 w-4" />
                    <AlertDescription>
                      Tests running for 14+ days have 85% higher confidence levels. 
                      Avoid ending tests prematurely to ensure statistical significance.
                    </AlertDescription>
                  </Alert>
                  <Alert>
                    <Target className="h-4 w-4" />
                    <AlertDescription>
                      Your highest performing variants use personalization tokens. 
                      Consider making this a standard practice across all campaigns.
                    </AlertDescription>
                  </Alert>
                </div>
              </div>
          ...
          <div className="space-y-6 mt-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input placeholder="e.g., Q1 Email Subject Line Test" className="mt-2" />
              </div>
              <div>
                ...
                <Select>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="smb-nurture">SMB Nurture Series</SelectItem>
