...
export const useReportsStore = create<ReportsState>((set, get) => ({
  ...
  fetchReport: async (period, customRange) => {
    ...
    try {
      ...
      const { start, end } =
        customRange || get().customDateRange || getDateRange(selectedPeriod);
      ...
      if (data.success) {
        ...
        const performanceScore = get().calculatePerformanceScore(
          report.metrics,
        );
  ...
  calculatePerformanceScore: (metrics) => {
    ...
    const conversionScore = Math.round(
      metrics.meetingConversionRate * 0.5 + metrics.winRate * 0.5,
    );
